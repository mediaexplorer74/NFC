...
namespace NfcSample //cc.troikadumper
{
	...
	public class Dump
	{
		...
		public virtual void write(Tag tag)
		{
			...
			{
				throw new IOException("Card UID mismatch: \n" + HexUtils.toString(tag.Id) + " (card) != " + HexUtils.toString(Uid) + " (dump)");
			}
		...
		protected internal virtual string makeFilename()
		{
			...
			return string.Format(FILENAME_FORMAT, now.Year + 1900, now.Month + 1, now.Date, now.Hour, now.Minute, now.Second, CardNumber, Balance);
		}
